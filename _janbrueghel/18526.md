---
pid: '18526'
label: The Drunkard pushed into the Pigsty (private collection)
object_type: Painting
genre:
worktags:
iconclass_code:
height_cm:
width_cm:
diameter_cm: '18'
location_country: Unavailable
location_city: Private Collection/Art Market
location_collection:
location_or_most_recent_sale_notes: Private Collection
collection_type: Private
realdate:
medium: Oil
medium_notes:
support:
support_notes:
signature:
signature_location:
support_marks:
further_inscription:
print_notes:
plate_dimensions:
states:
publisher:
series: Proverbs Series
collaborators:
collaborator_notes:
collectors_patrons:
our_attribution:
other_attribution_authorities: Pieter Bruegel II
bibliography: Maer, 1994, Ft. S. 192|AK Essen/Wien 1997/98, Abb. 2, S. 358|AK Antwerpen
  1998
biblio_reference: '8292'
exhibition_history:
ertz_1979:
ertz_2008:
bailey_walker:
hollstein_no:
bad_copy:
exclude_from_browsing:
provenance:
provenance_text: 1989, Paris, Gal. de Jonckheere, ausgestellt in Monte Carlo zur Biennale
  (Kat. 2)
related_works:
related_works_notes:
copies_and_variants:
curatorial_files:
general_notes:
image_file: objects/RKD66.0b.png
image_source:
framed_image:
external_resources:
thumbnail: img/derivatives/simple/18526/thumbnail.jpg
fullwidth: img/derivatives/simple/18526/fullwidth.jpg
collection: janbrueghel
layout: brueghel_item
order: '1563'
---
